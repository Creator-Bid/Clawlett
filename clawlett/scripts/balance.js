...
import { VERIFIED_TOKENS, ERC20_ABI } from './tokens.js'
...
async function main() {
  ...
  // Always show ETH (native balance)
  const ethBalance = await provider.getBalance(safeAddress)
  console.log(`ETH: ${formatAmount(ethBalance, 18, 'ETH')}`)

  if (args.all) {
    // Show all verified tokens with non-zero balance
    console.log('')
    for (const [symbol, address] of Object.entries(VERIFIED_TOKENS)) {
      // Skip native ETH (already shown above)
      if (address === NATIVE_ETH) continue
      try {
        const { balance, decimals, symbol: tokenSymbol } = await getTokenBalance(provider, safeAddress, address)
        if (balance > 0n) {
          console.log(`${tokenSymbol.padEnd(8)} ${formatAmount(balance, decimals, tokenSymbol)}`)
        }
      } catch {
        // Token might not exist or have issues - silently skip
      }
    }
  } else if (args.token) {
    // Show specific token
